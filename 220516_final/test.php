...
$con = mysqli_connect("localhost","root","", "cctv");

$sql = "select * from cctvdata";
...
if (mysqli_num_rows($result) > 0) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    $data_array[] =floatval($row["latitude"]);

    $data_array[] =floatval($row["longitude"]);
...
<script
	...
	src="https://apis.openapi.sk.com/tmap/jsv2?version=1&appKey=l7xxf71b7e3a0f8143d6b17183353dc91a56"></script>

<script type="text/javascript">

var chart_array = <?php echo $qqq; ?>;
  ...
  //출발지에서 도착지까지의 거리에서 11로 나눈값
    ...
    totallen  = parseInt((startPoint.getPosition().distanceTo(endPoint.getPosition()))/11);
    ...
    for(var i=0;i<rra.length;i++){ //총거리에서 11을 나눈값에서 근사값을 구하기 위한 배열
    ...
    arr.sort(function (a,b){return a-b; });
  	...
  	for (var i=0;i<arr.length;i++){
  	  ...
  	  if(tmp.length==5){  //근사값 거리에 있는 cctv5개 넣음
  ...
  var zlp="";
    ...
    // ans1+=","+String(chart_array[12]);
		...
		$
            ...
            sumDistance = parseFloat(((resultData[0].properties.totalDistance) / 1000).toFixed(1));

						sumTime =	parseFloat(((resultData[0].properties.totalTime) / 60).toFixed(0));
        ...
        // 3. 경로탐색 API 사용요청
    		...
    		$
                ...
                "searchOption" : 10
                 ...
                 sumDistance+=parseFloat(((resultData[0].properties.totalDistance) / 1000)

                    .toFixed(1));

 

                sumTime  +=  parseFloat(((resultData[0].properties.totalTime) / 60)
